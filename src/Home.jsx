...
function Home() {
  ...
  const sendEmail = (e) => {
    ...
    e.preventDefault();
    emailjs.sendForm('service_pal3yyb', 'template_9ajwk3o', form.current, 't39w1X1ZJJkZ0ovwi')
      .then((result) => {
      ...
      <section className="homesec3" id="latest">
        ...
        >
          {projects.slice(1, 6).map((project) => (
            <SwiperSlide key={project.id}><a href={project.link} target="_blank" rel="noreferrer"><img src={project.photo} alt="" /></a></SwiperSlide>
          ))}
        </Swiper>
      </section>
      {/* end third section */}
